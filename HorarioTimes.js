...
function calcularHorasSemana(trabajador) {
  ...
  dias.forEach(dia => {
    ...
    if (md?.length === 2 && md[0].trim() && md[1].trim()) {
      total += diffHoras(md[0], md[1]);
    }

    if (n?.length === 2 && n[0].trim() && n[1].trim()) {
      total += diffHoras(n[0], n[1]);
    }
  });
...
function renderWorkerTable(widget, trabajador) {
  ...
  // ⏱️ Próxima jornada / Fin / Descanso
  ...
  /* // ⏱️ Próxima jornada / Fin de jornada
  const minutos = minutosHastaProximaJornada(trabajador);
  ...
  if (minutos !== null) {
    ...
    if (m === 0) {
      ...
      tiempoTxt = `${h}:${String(m).padStart(2, "0")}m`;
    }
  ...
  // Espacio entre nombre y plaza
  titleRow.addSpacer(4);
  
  // Plaza (italic)
  const plazaText = titleRow.addText(trabajador.plaza);
  plazaText.font = Font.italicSystemFont(12);
  plazaText.textColor = new Color("#EFDECD", 0.7); // un poco más suave (opcional)
  ...
  diasSemanaOrdenados.forEach(dia => {
    ...
    cell.size = new Size(diaLibre ? 20 : 54, 35);
...
// En lugar de abrir el script, abre esta URL al tocar el widget
w.url = "https://ali-bhtty.web.app";
