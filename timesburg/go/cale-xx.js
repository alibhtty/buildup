...
for (let m = 0; m < 12; m++) {
  ...
  // Dibujar encabezado de días de la semana (L M X J V S D)
  ctx.setFont(Font.mediumSystemFont(fontSizeMonth * 0.8));
  ctx.setTextColor(CONFIG.colors.text);
  
  for (let i = 0; i < 7; i++) {
    const dayX = blockX + (i * DOT_SPACING);
    const dayY = blockY - (DOT_SPACING * 0.3);
