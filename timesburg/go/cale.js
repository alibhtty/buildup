...
// --- SOLO DIBUJAR EL MES ACTUAL ---
const m = currentMonth;

// Tamaños grandes y centrados
const bigDotSpacing = width / 10;
const bigDotRadius = bigDotSpacing * 0.38;

// Ancho real del mes: 7 columnas + diámetro del último círculo
const monthWidth = (6 * bigDotSpacing) + (bigDotRadius * 2);
const blockX = (width - monthWidth) / 2;  // CENTRADO REAL
const blockY = PADDING_TOP;

const fontSizeMonthBig = width * 0.08;
const fontSizeWeek = width * 0.045;
const fontSizeDayNumber = width * 0.028; // MÁS PEQUEÑO

// Título del mes (centrado)
ctx.setTextColor(CONFIG.colors.text);
ctx.setFont(Font.boldSystemFont(fontSizeMonthBig));
ctx.setTextAlignedCenter();
ctx.drawText(monthNames[m], new Point(width / 2, blockY - bigDotSpacing * 1.6));

// Encabezado L M X J V S D
ctx.setFont(Font.mediumSystemFont(fontSizeWeek));
ctx.setTextColor(CONFIG.colors.text);
ctx.setTextAlignedCenter();

for (let i = 0; i < 7; i++) {
  const dayCenterX = blockX + (i * bigDotSpacing) + bigDotRadius;
  const dayY = blockY - (bigDotSpacing * 0.3);
  ctx.drawText(weekDays[i], new Point(dayCenterX, dayY));
}

// Días del mes
const daysInMonth = new Date(currentYear, m + 1, 0).getDate();
let firstDayWeek = new Date(currentYear, m, 1).getDay();
let startOffset = (firstDayWeek + 6) % 7; // Lunes = 0
...
for (let d = 1; d <= daysInMonth; d++) {
  ...
  const dotX = blockX + (gridX * bigDotSpacing);
  const dotY = blockY + (gridY * bigDotSpacing);

  const fillColor = getDayColor(m, d);
  ctx.setFillColor(fillColor);
  ctx.fillEllipse(new Rect(dotX, dotY, bigDotRadius * 2, bigDotRadius * 2));

  // --- NÚMERO DENTRO DEL CÍRCULO ---
  ctx.setFont(Font.boldSystemFont(fontSizeDayNumber));
  ctx.setTextAlignedCenter();

  // Color del número: blanco SIEMPRE excepto si el círculo es blanco
  const textColor = (fillColor.hex === "#ffffff") ? new Color("#000000") : new Color("#ffffff");
  ctx.setTextColor(textColor);

  // Centrado perfecto
  ctx.drawText(
    d.toString(),
    new Point(dotX + bigDotRadius, dotY + bigDotRadius - fontSizeDayNumber * 0.32)
  );
}
...
// Ajustar tamaño para un solo mes (más grande)
const blockX = width * 0.08;
const blockY = PADDING_TOP;
const fontSizeMonthBig = width * 0.07;
const fontSizeWeek = width * 0.045;
const bigDotSpacing = width / 12;
const bigDotRadius = bigDotSpacing * 0.35;

// Título del mes
ctx.setTextColor(CONFIG.colors.text);
ctx.setFont(Font.boldSystemFont(fontSizeMonthBig));
ctx.setTextAlignedLeft();
ctx.drawText(monthNames[m], new Point(blockX, blockY - bigDotSpacing * 1.4));
...
for (let i = 0; i < 7; i++) {
  const dayX = blockX + (i * bigDotSpacing);
  const dayY = blockY - (bigDotSpacing * 0.3);
  ctx.drawText(weekDays[i], new Point(dayX, dayY));
}
