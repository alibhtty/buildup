...
# ✅ Minificación segura
npx terser source/$FILE.js \
  -c passes=3 \
  -m toplevel=true \
  --ecma 2022 \
  -o source/$FILE.min.js

# ✅ Ofuscación segura para Scriptable
npx javascript-obfuscator source/$FILE.min.js \
  --compact true \
  --string-array true \
  ...
  -o source/$FILE.obf.js

# ✅ Base64 final
base64 -i source/$FILE.obf.js -b 0 -o source/$FILE.b64
...
#./source/build.sh nombre        ARCHIVO  EJECUTAR ./timesburg/sant-pau/pipeline/build.sh
timesburg/sant-pau/pipeline/build.sh
